...
class ActionPointPaneRenderer implements IPrimitivePaneRenderer {
    ...
    draw(target: CanvasRenderingTarget2D) {
        target.useBitmapCoordinateSpace(scope => {
            ...
            const textYScaled = Math.round((this._p1.y + fontSize / 2 - this._options.lineWidth / 2) * scope.verticalPixelRatio);

            const x1 = Math.round((this._p1.x - this._options.width / 2) * scope.horizontalPixelRatio);
            const x2 = Math.round((this._p1.x + this._options.width / 2) * scope.horizontalPixelRatio);
    ...
    _drawTextLabel(scope: BitmapCoordinatesRenderingScope, text: string, x: number, y: number, left: boolean) {
        ...
        const leftAdjustment = left ? textWidth.width + offset * 4 + this._options.width / 2 : - this._options.width / 2;

        scope.context.fillStyle = this._options.labelBackgroundColor;
        scope.context.roundRect(x + offset - leftAdjustment, y - fontSize, textWidth.width + offset * 2,  fontSize + offset, 2);
        scope.context.fill();
        scope.context.beginPath();
